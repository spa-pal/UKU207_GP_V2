...
for(i=0;i<12;i++)
	{
	if(bps[i]._av)	avar_stat = ( (avar_stat & ~((0xffffffff>>(32-1))<<3+i)) | ((unsigned)1 << 3+i) );
	else	   		avar_stat = ( (avar_stat & ~((0xffffffff>>(32-1))<<3+i)) | ((unsigned)0 << 3+i) );
	}

for(i=0;i<6;i++)
	{
	if(av_inv[i])	avar_stat = ( (avar_stat & ~((0xffffffff>>(32-1))<<15+i)) | ((unsigned)1 << 15+i) );
	else	   		avar_stat = ( (avar_stat & ~((0xffffffff>>(32-1))<<15+i)) | ((unsigned)0 << 15+i) );
	}
...
for(i=0;i<4;i++)
	{
	if(sk_av_stat[i]==sasON)	avar_stat = ( (avar_stat & ~((0xffffffff>>(32-1))<<25+i)) | ((unsigned)1 << 25+i) );
	else	   		avar_stat = ( (avar_stat & ~((0xffffffff>>(32-1))<<25+i)) | ((unsigned)0 << 25+i) );
	}
