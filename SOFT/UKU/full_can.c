...
if((RXBUFF[1]==PUTTM1)&&((RXBUFF[0]&0x1f)>=0)&&((RXBUFF[0]&0x1f)<32))
     {
	can_debug_plazma[1][2]++;
...
if((RXBUFF[1]==PUTTM2)&&((RXBUFF[0]&0x1f)>=0)&&((RXBUFF[0]&0x1f)<32))
     {
     slave_num=RXBUFF[0]&0x1f;  
...
LPC_CAN1->EWL = 110; // Enter Normal Operating Mode
