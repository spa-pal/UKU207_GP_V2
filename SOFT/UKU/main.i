...
if(cnt_net_drv<=32) 
	...
	if(!bCAN_OFF)
			...
			mcp2515_transmit(cnt_net_drv,cnt_net_drv,GETTM_tr,bps[cnt_net_drv]._flags_tu,*((char*)(&bps[cnt_net_drv]._vol_u)),*((char*)((&bps[cnt_net_drv]._vol_u))+1),*((char*)(&bps[cnt_net_drv]._vol_i)),*((char*)((&bps[cnt_net_drv]._vol_i))+1));
...
if(a_ind . i==iMn)
...

 {
	...
	if(NUMIST==1) 
		...
		int2lcd(a_ind . i_s+1,'!',0);
		int2lcd(a_ind . i_s+2,'@',0);
		int2lcd(a_ind . i_s+3,'#',0);
	...
	int2lcdyx(bps[a_ind . s_i1]._build_year,2,12,0);
	int2lcdyx(bps[a_ind . s_i1]._build_month,2,15,0);
	int2lcdyx(bps[a_ind . s_i1]._build_day,2,18,0);
	
	sprintf(&lcd_buffer[29],"%d.%d.%d",bps[a_ind . s_i1]._hardvare_version,bps[a_ind . s_i1]._soft_version,bps[a_ind . s_i1]._build);
...
if(but==249)
	...
	if(but==254)
	     {
		 if((a_ind . s_i>=0)&&(a_ind . s_i<NUMIST))	tree_up(iFW_IPS,0,0,a_ind . s_i);
	     else tree_down(0,0);
	     ret(0);
	     }
	}
...
((SysTick_Type *) ((0xE000E000UL) + 0x0010UL))->LOAD = (SystemFrequency / 2000) - 1;
((SysTick_Type *) ((0xE000E000UL) + 0x0010UL))->CTRL = 0x07;
